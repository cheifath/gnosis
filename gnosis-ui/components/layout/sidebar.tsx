...
const navItems = [
  ...
  { href: "/installations", label: "Installations", icon: Download },
  { href: "/repositories", label: "Repositories", icon: FolderGit2 },
  { href: "/pull-request", label: "Pull Requests", icon: GitPullRequest },
...
export default function Sidebar() {
  ...
  return (
    <div className="w-[272px] bg-gradient-to-b from-slate-900/95 via-slate-900/90 to-slate-950/95 backdrop-blur-xl border-r border-white/[0.06] h-screen flex flex-col relative">
      ...
      <div className="absolute top-0 right-0 w-px h-full bg-gradient-to-b from-purple-500/20 via-transparent to-blue-500/20" />
      ...
      <div className="p-6 pb-2">
        <Link href="/dashboard" className="flex items-center gap-3 group">
          <div className="relative">
            {/* Outer glow ring */}
            <div className="absolute -inset-1.5 rounded-2xl bg-gradient-to-br from-purple-500 via-indigo-500 to-cyan-500 opacity-0 group-hover:opacity-25 blur-md transition-all duration-500" />
            {/* Logo container */}
            <div className="relative w-10 h-10 rounded-2xl bg-gradient-to-br from-purple-500 via-violet-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-500/25 group-hover:shadow-purple-500/50 group-hover:scale-105 transition-all duration-300 ring-1 ring-white/10">
              {/* Inner icon with layered effect */}
          ...
          <div className="flex flex-col">
            <span className="text-[17px] font-extrabold tracking-wide bg-gradient-to-r from-white via-purple-100 to-slate-300 bg-clip-text text-transparent group-hover:from-purple-200 group-hover:via-white group-hover:to-indigo-200 transition-all duration-300">
              GNOSIS
            </span>
            <span className="text-[9px] font-medium tracking-[0.25em] text-slate-500 -mt-0.5 group-hover:text-purple-400/70 transition-colors duration-300">
              CODE REVIEW AI
            </span>
          </div>
        </Link>
      ...
      <nav className="flex-1 px-3 space-y-0.5">
        ...
        {navItems.map((item) => {
          const isActive = pathname === item.href || pathname.startsWith(item.href + "/") || (item.href === "/repositories" && pathname.startsWith("/repository/")) || (item.href === "/installations" && pathname.startsWith("/installations/"))
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-200 ${
                isActive
                  ? "bg-gradient-to-r from-purple-500/15 to-indigo-500/10 text-white shadow-sm"
      ...
      <div className="p-3 pt-0">
        ...
        {/* Role badge */}
        {isAdmin && (
          <div className="flex items-center gap-2 px-3 py-2 mb-2 rounded-xl bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border border-purple-500/15">
            ...
            <span className="text-[11px] font-semibold text-purple-300 tracking-wide">Admin Access</span>
          </div>
        )}
        ...
        <div className="relative">
          ...
          >
            <div className="relative">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-sm font-semibold text-white overflow-hidden ring-2 ring-white/[0.06] group-hover:ring-purple-500/20 transition-all">
                {user?.avatar_url ? (
